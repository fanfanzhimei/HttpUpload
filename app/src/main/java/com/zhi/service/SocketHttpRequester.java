...
public class SocketHttpRequester {
    ...
    public static byte[] postXml(String path, String xml, String encoding) throws Exception {
        ...
        if (conn.getResponseCode() == 200) {
            return StreamTool.readStream(conn.getInputStream());
        }
        return null;
    }
     ...
     * <FORM METHOD=POST ACTION="http://192.168.1.3:8080/FileUpload/UploadServlet" enctype="multipart/form-data">
    ...
    public static boolean post(String path, Map<String, String> params, FormFile[] files) throws Exception {
        ...
        final String endline = "--" + BOUNDARY + "--/r/n";   //数据结束标志"---------------------------7da2137580612--"
        //下面两个for循环都是为了得到数据长度参数，依据表单的类型而定  ，首先得到文件类型数据的总长度(包括文件分割线)
        ...
        StringBuilder textEntity = new StringBuilder();  //构造文本类型参数的实体数据
        ...
        Socket socket = new Socket(InetAddress.getByName(url.getHost()), port);  //建立一个Socket链接
        OutputStream outStream = socket.getOutputStream(); //获得一个输出流（从Android流到web）
        String requestMethod = "POST " + url.getPath() + " HTTP/1.1\r\n";  //下面完成HTTP请求头的发送
        outStream.write(requestMethod.getBytes());

        String accept = "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8\r\n";
        String language = "Accept-Language: zh-CN,zh;q=0.8\r\n";
        String contentType = "Content-Type: multipart/form-data; boundary=" + BOUNDARY + "\r\n";
        String contentLength = "Content-Length: " + dataLength + "\r\n";
        String alive = "Connection: Keep-Alive\r\n";
        String host = "Host: " + url.getHost() + ":" + port + "\r\n";

        outStream.write(accept.getBytes());  //构建accept
        outStream.write(language.getBytes());  //构建language
        outStream.write(contentType.getBytes());  //构建contenttype
        outStream.write(contentLength.getBytes()); //构建contentlength
        outStream.write(alive.getBytes());  //构建alive
        outStream.write(host.getBytes());   //构建host
        outStream.write("\r\n".getBytes());  //写完HTTP请求头后根据HTTP协议再写一个回车换行

        outStream.write(textEntity.toString().getBytes());   //把所有文本类型的实体数据发送出来

        for (FormFile uploadFile : files) {  //把所有文件类型的实体数据发送出来
            ...
            if (uploadFile.getInStream() != null) {  // 边读边写
        ...
        outStream.write(endline.getBytes());   //发送数据结束标志，表示数据已经结束
...
//    }
    ...
    public static byte[] post(String path, Map<String, String> params, String encode) throws Exception {
        ...
        URL url = new URL(path);
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setDoOutput(true);  //设置允许对外发送请求参数
        conn.setUseCaches(false);  //设置不进行缓存
        conn.setConnectTimeout(5 * 1000);
        conn.setRequestMethod("POST");
        //下面设置http请求头
